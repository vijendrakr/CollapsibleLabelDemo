...
//  Few addition by Vijendra kr.

import UIKit

protocol refreshTVCDelegate {
...
class CollapsibleLabel: UILabel {
    ...
    var delegate: refreshTVCDelegate?
    ...
    private func lblAttributedString(for labelText:String, with actionText:String)  {
        ...
        //ful string attr
        let attribs = [NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.font: UIFont.systemFont(ofSize: 14.0)]
        let attributedString: NSMutableAttributedString = NSMutableAttributedString(string: fullText, attributes: attribs)
        
        //btn string attr
        attributedString.addAttribute(NSAttributedString.Key.link, value: actionText, range: NSRange(location: labelText.utf16.count, length: actionText.utf16.count))
        attributedString.addAttribute(NSAttributedString.Key.underlineStyle, value: NSUnderlineStyle.single.rawValue, range: NSRange(location: labelText.utf16.count, length: actionText.utf16.count))
    ...
    @objc private func tapFunction(sender:UITapGestureRecognizer) {
        ...
        case true:
            ...
            if isLessTapped(in: sender) {
                lessClicked?()
                toggleIsExpanded()
                delegate?.refreshCellAction(index: self.tag)
